...
app.use(cors());
app.use(express.static(path.join(__dirname, "public"))); // ให้เสิร์ฟไฟล์ frontend

// ตรวจสอบและสร้างไฟล์ JSON ถ้ายังไม่มี
if (!fs.existsSync("students.json")) fs.writeFileSync("students.json", JSON.stringify([]));
if (!fs.existsSync("paid.json")) fs.writeFileSync("paid.json", JSON.stringify([]));
